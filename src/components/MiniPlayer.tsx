  ...
  return (
    <div className="mini-player-container">
      ...
      <div className="mini-player" role="region" aria-label="Audio player">
        <button className="mp-btn mp-play" onClick={toggle} aria-label={isPlaying ? 'Pause' : 'Play'}>{isPlaying ? '⏸︎' : '▶︎'}</button>
        <div className="mp-bar" onClick={(e) => { const rect = (e.currentTarget as HTMLDivElement).getBoundingClientRect(); const pct = (e.clientX - rect.left) / rect.width; if (dur > 0 && isFinite(dur)) seek(dur * Math.max(0, Math.min(1, pct))); }}>
